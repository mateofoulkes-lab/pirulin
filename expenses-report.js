...
function broadCategory(it){
  ...
  // Mascota primero: evita que veterinaria de Angu caiga en Salud.
  ...
  // Comidas preparadas, salidas y antojos.
  ...
  // Viajes/ocio: después de comida para que una cena de vacaciones siga siendo comida.
...
function grouped(items,keyFn){
  const map=new Map();
  for(const it of items){const key=keyFn(it);if(!key)continue;const row=map.get(key)||{label:key,amount:0,count:0};row.amount+=Number(it.amount)||0;row.count++;map.set(key,row)}
  return [...map.values()].sort((a,b)=>b.amount-a.amount);
}
function aggregateSmall(rows,max=7){
  if(rows.length<=max)return rows;
  const keep=rows.slice(0,max-1),rest=rows.slice(max-1).reduce((a,r)=>({label:'Otros',amount:a.amount+r.amount,count:a.count+r.count}),{amount:0,count:0});
  return [...keep,rest];
}
...
async function readReportItems(){
  ...
  do{
    ...
    ['cat','amount','amountPending','settlement','date'].forEach(f=>params.append('mask.fieldPaths',f));
...
function installReport(){
  ...
  suite.insertAdjacentHTML('beforeend',`<div class="gastos-modal" id="expenseReportModal"><div class="gastos-modal-card rp-card"><div class="gastos-modal-head"><div><div class="eyebrow">GASTOS</div><h3>Reporte</h3></div><button class="modal-close" id="expenseReportClose">×</button></div><div class="rp-controls"><div class="rp-modes" id="expenseReportModes"><button data-mode="month" class="active">Mes</button><button data-mode="year">Año</button><button data-mode="all">Todo</button></div><div class="rp-period-nav"><button id="expenseReportPrev">‹</button><strong id="expenseReportPeriod"></strong><button id="expenseReportNext">›</button></div></div><div id="expenseReportBody"><div class="rp-loading">Armando reporte…</div></div></div></div>`);
  const style=document.createElement('style');style.textContent=`
  #expenseReportModal{z-index:120!important}.rp-card{width:min(720px,calc(100vw - 24px))!important;max-height:min(88vh,900px)!important;overflow:auto!important;padding-bottom:22px!important}.rp-controls{display:flex;gap:10px;align-items:center;justify-content:space-between;flex-wrap:wrap;margin:8px 0 18px}.rp-modes{display:flex;background:#eef1f6;padding:3px;border-radius:12px}.rp-modes button{border:0;background:transparent;padding:8px 13px;border-radius:9px;font-weight:800;color:#7b8492}.rp-modes button.active{background:#fff;color:#262b34;box-shadow:0 2px 9px rgba(38,43,52,.09)}.rp-period-nav{display:flex;align-items:center;gap:8px}.rp-period-nav button{border:0;background:#eef1f6;width:34px;height:34px;border-radius:10px;font-size:23px;color:#697282}.rp-period-nav strong{min-width:128px;text-align:center;font-size:13px}.rp-chart-wrap{display:grid;grid-template-columns:minmax(190px,260px) 1fr;gap:24px;align-items:center}.rp-donut{width:min(58vw,240px);aspect-ratio:1;border-radius:50%;margin:auto;position:relative;display:grid;place-items:center;cursor:pointer}.rp-donut:after{content:'';position:absolute;width:58%;height:58%;background:#fff;border-radius:50%;box-shadow:0 0 0 1px rgba(0,0,0,.025)}.rp-donut>div{position:relative;z-index:2;text-align:center;max-width:55%}.rp-donut strong{display:block;font-size:17px;line-height:1.05}.rp-donut small{font-size:10px;color:#8a93a0}.rp-legend{display:flex;flex-direction:column;gap:3px}.rp-legend-row{display:grid;grid-template-columns:12px minmax(0,1fr) auto;align-items:center;gap:9px;border:0;background:transparent;padding:7px 6px;border-radius:10px;text-align:left;color:#303642}.rp-legend-row[data-category]{cursor:pointer}.rp-legend-row[data-category]:active{background:#f1f3f7}.rp-dot{width:10px;height:10px;border-radius:50%}.rp-name{font-size:12px;font-weight:800}.rp-value{text-align:right}.rp-value b{display:block;font-size:11px}.rp-value small{display:block;font-size:9px;color:#9199a5}.rp-stats{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-top:20px}.rp-stats>div{background:#f6f7fa;border-radius:14px;padding:12px}.rp-stats small{display:block;font-size:9px;text-transform:uppercase;letter-spacing:.04em;color:#939ba7;font-weight:800}.rp-stats strong{display:block;margin-top:4px;font-size:13px}.rp-tip{text-align:center;color:#929aa6;font-size:10px;margin-top:14px}.rp-back{border:0;background:transparent;color:#6376c9;font-weight:800;padding:0 0 6px}.rp-kicker{font-size:21px;font-weight:900;margin:4px 0 14px}.rp-loading,.rp-empty{text-align:center;padding:45px 15px;color:#8b94a0;font-size:13px}@media(max-width:560px){.rp-card{max-height:92vh!important}.rp-chart-wrap{grid-template-columns:1fr;gap:15px}.rp-donut{width:min(55vw,220px)}.rp-stats{grid-template-columns:1fr 1fr}.rp-controls{justify-content:center}.rp-period-nav{width:100%;justify-content:center}.rp-legend-row{padding:6px 4px}}
