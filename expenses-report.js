...
function broadCategory(it){
  const shared=window.PirulinExpenseCategories?.classify?.(it);
  if(shared)return shared;
  const c=String(it.cat||'').trim();
  return c||'Otros / regalos';
}
function canonicalMerchant(desc){
  ...
  const cleaned=d.replace(/\b(comida|cosas|gasto|gastos|pago|compra|compras|mateo|dani|ultimo|ultima|muchos|muchas|reserva)\b/g,' ').replace(/\s+/g,' ').trim();
...
function aggregateSmall(rows,max=7){
  ...
  const rest=rows.slice(max-1).reduce((a,r)=>({label:'Otros',amount:a.amount+r.amount,count:a.count+r.count,mateo:a.mateo+r.mateo,dani:a.dani+r.dani}),{amount:0,count:0,mateo:0,dani:0});
  return [...keep,rest];
}
function conic(rows,total){
  let acc=0;
  return rows.map((r,i)=>{const from=acc,to=acc+(total?100*r.amount/total:0);acc=to;return `${REPORT_COLORS[i%REPORT_COLORS.length]} ${from}% ${to}%`}).join(',');
}
...
function renderReport(){
  ...
  const prev=$rp('#expenseReportPrev'),next=$rp('#expenseReportNext');
  if(prev)prev.style.visibility=reportMode==='all'?'hidden':'visible';if(next)next.style.visibility=reportMode==='all'?'hidden':'visible';
  ...
  if(reportCategory){
    ...
    const rows=aggregateSmall(grouped(catItems,it=>canonicalMerchant(it.desc)),7);
    ...
    const rows=aggregateSmall(grouped(eligible,broadCategory),10),top=rows[0];
    ...
    $rp('.rp-donut')?.addEventListener('click',e=>{
      const rect=e.currentTarget.getBoundingClientRect(),x=e.clientX-rect.left,y=e.clientY-rect.top;
      const ang=(Math.atan2(y-rect.height/2,x-rect.width/2)*180/Math.PI+450)%360;let acc=0;
      for(const r of rows){acc+=360*r.amount/total;if(ang<=acc){if(r.label!=='Otros'){reportCategory=r.label;renderReport()}break}}
    });
  }
}
...
async function openReport(){
  const modal=$rp('#expenseReportModal');modal?.classList.remove('rp-closing');modal?.classList.add('show');reportCategory=null;
...
function shiftPeriod(delta){
  if(reportMode==='year')reportCursor=new Date(reportCursor.getFullYear()+delta,0,1);else reportCursor=new Date(reportCursor.getFullYear(),reportCursor.getMonth()+delta,1);
  reportCategory=null;renderReport();
}
function installReport(){
  ...
  const btn=document.createElement('button');btn.id='openExpenseReport';btn.type='button';btn.textContent='Reporte';menu.prepend(btn);
  btn.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();menu.classList.remove('show');openReport()});
