...
function setMockCategories(categories){
  ...
  try{window.eval(`(()=>{const data=${jsonForEval(data)};noteCategories.splice(0,noteCategories.length,...data);if(typeof selectedNoteFilters!=='undefined'){const valid=new Set(data.map(c=>c.id));[...selectedNoteFilters].forEach(id=>{if(!valid.has(id))selectedNoteFilters.delete(id)})}if(typeof renderNoteFilters==='function')renderNoteFilters();if(typeof renderNotes==='function')renderNotes();if(typeof renderCategoryEditor==='function'&&document.getElementById('categoryEditorModal')?.classList.contains('show'))renderCategoryEditor();})()`)}catch(e){console.error('Pirulín render note categories',e)}
}
function relabelSharedUI(){
  const other=otherPerson();
  qa('#notesGrid .note-card').forEach(card=>{const note=latestNotes.get(card.dataset.noteId);const chip=q('.note-chip',card);if(note?.shared&&chip)chip.textContent=`Con ${other}`});
  const label=q('#noteShareInput')?.closest('.note-pref')?.querySelector('span');if(label)label.textContent=`Compartir con ${other}`;
}

function readEditingPayload(){
  const title=q('#noteTitleInput')?.value?.trim()||'',body=q('#noteBodyInput')?.value?.trim()||'',drawing=safeEval('editingDrawing','')||'';
  if(!title&&!body&&!drawing)return null;
  const editingId=safeEval('editingNoteId',null),tags=safeEval('[...editingNoteTags]',[])||[],old=editingId?latestNotes.get(String(editingId)):null;
  return {id:String(editingId||`n${Date.now()}-${Math.random().toString(36).slice(2,6)}`),title:title||'Sin título',body,drawing,tags,shared:!!q('#noteShareInput')?.checked,pinned:!!q('#notePinInput')?.checked,createdBy:old?.createdBy||window.PirulinFirebase?.person||null,createdByUid:old?.createdByUid||window.PirulinFirebase?.user?.uid||null,previousShared:old?.shared??false};
}
async function saveEditingNote(){
  const payload=readEditingPayload();if(!payload){say('Escribí algo');return}
  try{const {previousShared,...note}=payload;await window.PirulinNotes.saveNote(note,{previousShared});q('#noteModal')?.classList.remove('show');safeEval('editingNoteId=null;editingNoteTags=new Set();editingDrawing=""');say(note.shared?`Nota guardada y compartida con ${otherPerson()}`:'Nota guardada');window.dispatchEvent(new CustomEvent('pirulin-note-saved',{detail:{id:note.id,shared:note.shared}}))}catch(e){console.error('Pirulín save note',e);say('No pude guardar la nota')}
}
...
function openNoteMenu(card,button){const id=card?.dataset?.noteId,note=id?latestNotes.get(id):null;if(!note)return;const m=ensureNoteMenu();m.dataset.noteId=id;m.innerHTML=menuButton('Editar','edit')+menuButton(note.pinned?'Desfijar':'Fijar arriba','pin')+menuButton(note.shared?'Descompartir':`Compartir con ${otherPerson()}`,'share')+menuButton('Eliminar','delete',true);const r=button.getBoundingClientRect();m.style.left=`${Math.max(8,Math.min(window.innerWidth-198,r.right-190))}px`;m.style.top=`${Math.min(window.innerHeight-210,r.bottom+5)}px`;m.style.display='block'}
async function actOnNote(action,id){
  const note=latestNotes.get(id);if(!note)return;closeNoteMenu();
...
async function syncCategoriesFromMock(){
  if(syncingCategories||!window.PirulinNotes)return;syncingCategories=true;
  try{const current=mockCategories(),cur=new Map(current.map(c=>[String(c.id),c])),writes=[];cur.forEach((c,id)=>{const old=latestCategories.get(id);if(!old||old.name!==c.name||(old.parent||null)!==(c.parent||null))writes.push(window.PirulinNotes.saveCategory(c))});latestCategories.forEach((c,id)=>{if(!cur.has(id))writes.push(window.PirulinNotes.deleteCategory(id))});await Promise.all(writes)}catch(e){console.error('Pirulín sync note categories',e);say('No pude guardar las categorías')}finally{syncingCategories=false}
}
...
function installHooks(){
  if(document.documentElement.dataset.pirulinNotesHooks==='1')return;document.documentElement.dataset.pirulinNotesHooks='1';ensureNoteMenu();ensureHeaderMenu();
  if(q('#notesGrid'))new MutationObserver(relabelSharedUI).observe(q('#notesGrid'),{childList:true,subtree:true});
